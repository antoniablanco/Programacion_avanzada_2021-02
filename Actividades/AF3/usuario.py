...
class Usuario:
    ...
    def ver_videos(self, nombre_lista):
        for pelicula in nombre_lista:
            pelicula.reproducir()
            calificacion = self.calcular_pelicula(pelicula)
            if calificacion:
                print("Le ha gustado la pelicula, era lo que esperaba")
            else:
                print("No le ha gustado la pelicula, esperaba mas de ella")
            yield calificacion, pelicula.rankings

        print("Se ha acabado la lista de reproduccion")
