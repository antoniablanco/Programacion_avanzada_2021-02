...
class Cliente:
    ...
    def thread_escuchar(self):
        ...
        try:
            while True:
                ...
                response_length = int.from_bytes(largo, byteorder='big')
                response = bytearray()
                ...
                self.manejar_comando(decodificado)
    ...
    def enviar(self, msg):
        # Completar
        codificado = self.codificar_mensaje(msg)
        largo = len(codificado).to_bytes(4, byteorder='big')
        self.socket_cliente.sendall(largo + codificado)
