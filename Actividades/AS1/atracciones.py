...
class AtraccionAdrenalinica(Atraccion):
    ...
    def efecto_atraccion(self, persona):
        ...
        if persona.salud < self.salud_necesaria:
            print("No tienes la salud necesaria, te han bajado del juego")
...
class AtraccionAcuatica(AtraccionFamiliar):
    ...
    def ingresar_persona(self, persona):
        ...
        if persona.tiene_pase:
            ...
            print("No tienes el pase, no puedes subirte a este juego")


# Recuerda completar la herencia!
class MontanaAcuatica(AtraccionAdrenalinica, AtraccionAcuatica):
    # Falta que herede correctamente
    ...
    def iniciar_juego(self, personas):
        ...
        for k in range(len(personas)):
            print(f'{personas[k].nombre} has jugado en la MontanaAcuatica')
            personas[k].esperando = False

            if personas[k].salud <= self.salud_necesaria * self.dificultad:
                print("Has caido al agua")
                personas[k].tiene_pase = False
        
        AtraccionAdrenalinica.efecto_atraccion(self, personas)
