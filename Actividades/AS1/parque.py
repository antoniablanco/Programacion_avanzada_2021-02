class Parque:
    ...
    def distribuir_personas(self):
        # COMPLETAR
        for k in range(len(self.personas)):
            if self.personas[k].revision_juegos():
                juego = self.personas[k].siguiente_juego()
                self.atracciones[juego].ingresar_persona(self.personas[k])
