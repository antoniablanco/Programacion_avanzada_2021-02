...
class Persona(ABC):
    ...
    def siguiente_juego(self):
        proximo_juego = self.juegos.pop(0)
        return proximo_juego
...
class Adulto(Persona):
    ...
    def definir_estados(self):
        salud = self.edad * randint(1, 3)
        felicidad = self.edad * self.dinero
        self.salud = salud
...
class Nino(Persona):
    ...
    def definir_estados(self):
        salud = self.edad * randint(1, 35)
        felicidad = 10 * len(self.padre)
        self.salud = salud
        self.felicidad = felicidad
